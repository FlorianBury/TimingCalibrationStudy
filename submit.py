...
CMSSW_DIR = '/nfs/scratch/fynu/fbury/UpgradeCalibrations/CMSSW_11_2_0_pre7/src/SimTracker/SiPhase2TimingCalibration/test/'
SETUP_CMSSW = (
               "module --force purge;"
               "module load cp3;"
...
RUN_SCRIPT = 'PythiaGunCalibration_cfg.py'
HARVESTER_SCRIPT = 'Harvester_cfg.py'
SLURM_OUTPUT_DIR = '/nfs/scratch/fynu/fbury/UpgradeCalibrations/slurm/production/'
...
class SubmitScans:
    ...
    def run(cls,args):
        ...
        dqm_cmd = cls.format_command(['cmsRun',os.path.join(CMSSW_DIR,RUN_SCRIPT)] + args)
    ...
    def submit_on_slurm(self,name,debug=False):
        ...
        #config.cmsswDir = CMSSW_DIR.split('src')[0]+'src'
        ...
        base_payload = "python submit.py --run " + " ".join([n+"={"+n+"}" for n in self.inputParamsNames])
